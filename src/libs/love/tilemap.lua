...
---@field private imageManager ImageManager The ImageManager of the map.
...
---@param imgManager ImageManager The ImageManager of the scene.
---@return Tilemap tilemap The Tilemap object.
function tilemap.newTilemap(scenePath, imgManager)
    ---@type Tilemap
    local self = {
...
function Tilemap:draw()
    for _, layer in ipairs(self.scene.layers) do
        ...
        local tileSize = self.imageManager:getGrid(1):getTileSize()
        for y = 1, height do
            for x = 1, width do
                local tileIndex = x + ((y - 1) * width)
                local quadIndex = mapData[tileIndex]
                if quadIndex ~= 0 then
                    love.graphics.draw(self.imageManager:getImage(), self.imageManager:getGrid(1):getQuad(quadIndex),
...
function Tilemap:getGrid()
    return self.imageManager:getGrid(1)
end

---Gets the ImageManager of the tilemap.
---@return ImageManager imageManager The ImageManager of the map.
function Tilemap:getImageManager()
...
function Tilemap:getMapSizeInPixels()
    return self.mapSize * self.imageManager:getGrid(1):getTileSize()
end
