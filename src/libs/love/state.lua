...
function StateMachine:addState(state)
	if self.states[state:getName()] then
		error("State '" .. state:getName() .. "' already exists.")
	end
...
function StateMachine:changeState(state, ...)
	if not self.states[state:getName()] then
		error("State '" .. state:getName() .. "' does not exist.")
	end
